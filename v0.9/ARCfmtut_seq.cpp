...
int turnSelectionToGreyscale(char filename[], int radius, Image& image)
{
	int H = image.H, W = image.W;
	int size = H*W;
	double centerAH = H/2-1;
	double centerAW = W/2-1;
	double centerBH = H/2-1;
	double centerBW = W/2;
	double centerCH = H/2;
	double centerCW = W/2-1;
	double centerDH = H/2;
	double centerDW = W/2;
	...
	int posH = 0;
	int posW = 0;
	int circle_limit = radius * radius;
	int circle[size];
	Image greyImage(H, W);
	...
	if ((H*W)%2==0)
	{
		for (int i=0;i<size;i++)
		{
			posH = i/W;
			posW = i%W;
			double distanceH = centerDH-posH;
			double distanceW = centerDW-posW;
	...
	if ((H*W)%2==0)
	{
		for (int i=0;i<size;i++)
		{
			...
			if ((posW >= centerAW)&&(posH >= centerAH))
			{
				distanceH = centerAH-posH;
				distanceW = centerAW-posW;
				pixelX[i] = (int) centerAW + ceil((cos(radians) * distanceW) - (sin(radians) * distanceH));
				pixelY[i] = (int) centerAH + ceil((sin(radians) * distanceW) + (cos(radians) * distanceH));
			}
			if ((posW <= centerBW)&&(posH >= centerBH))
			{
				distanceH = centerBH-posH;
				distanceW = centerBW-posW;
				pixelX[i] = (int) centerBW + ceil((cos(radians) * distanceW) - (sin(radians) * distanceH));
				pixelY[i] = (int) centerBH + ceil((sin(radians) * distanceW) + (cos(radians) * distanceH));
			}
			if ((posW >= centerCW)&&(posH <= centerCH))
			{
				distanceH = centerCH-posH;
				distanceW = centerCW-posW;
				pixelX[i] = (int) centerCW + ceil((cos(radians) * distanceW) - (sin(radians) * distanceH));
				pixelY[i] = (int) centerCH + ceil((sin(radians) * distanceW) + (cos(radians) * distanceH));
			}
			if ((posW <= centerDW)&&(posH <= centerDH))
			{
				pixelX[i] = (int) centerDW + ceil((cos(radians) * distanceW) - (sin(radians) * distanceH));
				pixelY[i] = (int) centerDH + ceil((sin(radians) * distanceW) + (cos(radians) * distanceH));
			}
		}
	} else
	{
